...
!1}d=textureUnit=0;for(e=B.length;d<e;d++)if(r=B[d],v=r.scopeId.value,v instanceof pc.Texture)s=v,this.setTexture(s,textureUnit),r.slot!==textureUnit&&(c.uniform1i(r.locationId,textureUnit),r.slot=textureUnit),textureUnit++;else{r.array.length=0;numTexures=v.length;for(g=0;g<numTexures;g++)s=v[g],this.setTexture(s,textureUnit),r.array[g]=textureUnit,textureUnit++;c.uniform1iv(r.locationId,r.array)}d=0;for(e=x.length;d<e;d++)if(B=x[d],g=B.scopeId,r=B.version,v=g.versionObject.version,r.globalId!==
...
!1;break}if(f)return d+1}d=this._mapXForms[c].length;this._mapXForms[c][d]=[];for(e=0;e<a.data.length;e++)this._mapXForms[c][d][e]=a.data[e];return d+1},updateShader:function(a,c,d){var e;e=c._lights;this._mapXForms=[];var f=a.extTextureLod&&16>a.samplerCount,k=this.prefilteredCubeMap128||c.skyboxPrefiltered128,m=this.prefilteredCubeMap64||c.skyboxPrefiltered64,h=this.prefilteredCubeMap32||c.skyboxPrefiltered32,n=this.prefilteredCubeMap16||c.skyboxPrefiltered16,l=this.prefilteredCubeMap8||c.skyboxPrefiltered8,
r=this.prefilteredCubeMap4||c.skyboxPrefiltered4;if(k){var v=k&&m&&h&&n&&l&&r;f?6>k._levels.length?v?this.setParameter("texture_prefilteredCubeMap128",k):console.log("Can't use prefiltered cubemap: "+v+", "+f+", "+k._levels):this.setParameter("texture_prefilteredCubeMap128",k):v?(this.setParameter("texture_prefilteredCubeMap128",k),this.setParameter("texture_prefilteredCubeMap64",m),this.setParameter("texture_prefilteredCubeMap32",h),this.setParameter("texture_prefilteredCubeMap16",n),this.setParameter("texture_prefilteredCubeMap8",
...
pc.Vec3.ZERO:this.meshInstance.node.getPosition();m=this.camera?this.camera._node.getPosition():pc.Vec3.ZERO;for(d=0;d<this.numParticles;d++){var l=Math.floor(this.vbCPU[4*d*this.numParticleVerts+3]),n=this.particleTex[4*d+0+8*this.numParticlesPot];u.x=n;u.y=this.particleTex[4*d+1+8*this.numParticlesPot];u.z=this.particleTex[4*d+2+8*this.numParticlesPot];var r=pc.math.lerp(this.rate,this.rate2,n),z=this.lifetime,G=this.particleTex[4*l+3+4*this.numParticlesPot]+b,M=e(G/z),P=0,W=0,Z=0<G&&G<z;if(Z){s.data=
...
k),this.material.setParameter("particleTexIN",d),this.swapTex=!this.swapTex;if(!this.loop&&this.onFinished&&Date.now()>this.endTime)this.onFinished();h.setDepthTest(!0);h.setDepthWrite(!0)}};return{ParticleEmitter:M}}());pc.extend(pc,function(){var e=function(a,b,c){this.device=a;a=a.getProgramLibrary();this.pickProgStatic=a.getProgram("pick",{skin:!1});this.pickProgSkin=a.getProgram("pick",{skin:!0});this.pickColor=new Float32Array(4);this.scene=null;this.clearOptions={color:[1,1,1,1],depth:1,flags:pc.CLEARFLAG_COLOR|pc.CLEARFLAG_DEPTH};this.resize(b,c)};e.prototype.getSelection=function(a){var b=this.device;a.width=a.width||1;a.height=a.height||1;var c=b.getRenderTarget();b.setRenderTarget(this._pickBufferTarget);
...
k.setSource(a.loadedTextures.slice(0,6).map(function(a){return a.getSource()}));f.push(k);if(a.loadedTextures&&6<a.loadedTextures.length){var k=a.loadedTextures[6],m=128;for(i=0;6>i;i++){var h=new pc.gfx.Texture(this._device,{cubemap:!0,fixCubemapSeams:!0,autoMipmap:!0,format:k.format,rgbm:k.rgbm,width:m,height:m});h.addressU=pc.ADDRESS_CLAMP_TO_EDGE;h.addressV=pc.ADDRESS_CLAMP_TO_EDGE;m*=0.5;h._levels[0]=k._levels[i];h.upload();f.push(h)}}e.off("change",this._onCubemapAssetChanged,this);e.on("change",
this._onCubemapAssetChanged,this);return f};a.prototype._onCubemapAssetChanged=function(a,b,e,f){var k=this;if("data"===b){b=!1;if(e.textures.length!==f.textures.length)b=!0;else for(var m=0;m<e.textures.length;m++)if(e.textures[m]!==f.textures[m]){b=!0;break}b&&k._loadCubemapImages(a).then(function(b){var d=a.resources.slice(0);b&&(a.resource.setSource(b.map(function(a){return a.getSource()})),a.fire("change",a,"resources",a.resources,d))});a.resource.minFilter=e.minFilter;a.resource.magFilter=e.magFilter;
a.resource.addressU=pc.ADDRESS_CLAMP_TO_EDGE;a.resource.addressV=pc.ADDRESS_CLAMP_TO_EDGE;a.resource.anisotropy=e.anisotropy;a.resource.upload()}else"file"===b&&(f&&(url=f.url,delete k._assets._urls[url],k._assets.loader.removeFromCache(url),k._assets.loader.unregisterHash(url)),e?(url=a.getFileUrl(),k._assets._urls[url]=a.id,k._assets.loader.registerHash(e.hash,url),k._assets.loader.request(new pc.resources.TextureRequest(url)).then(function(b){var b=b[0],d=128,e=a.resources.slice(0);a.resources=
[a.resource];for(m=0;6>m;m++){var f=new pc.gfx.Texture(k._device,{cubemap:!0,fixCubemapSeams:!0,autoMipmap:!0,format:b.format,rgbm:b.rgbm,width:d,height:d}),d=0.5*d;f._levels[0]=b._levels[m];f.upload();a.resources.push(f)}a.fire("change",a,"resources",a.resources,e)},function(a){console.error("Could not load prefiltered cubemap: "+a)})):(e=a.resources.slice(0),a.resources=[a.resource],a.fire("change",a,"resources",a.resources,e)))};a.prototype._areValidImages=function(a){var b=a&&6===a.length,e;if(b)for(var f=
a[0]?a[0].width:null,k=a[0]?a[0].height:null,m=0;6>m;m++){if(!a[m]){b=!1;break}if(!a[m]instanceof HTMLCanvasElement||!a[m]instanceof HTMLImageElement||!a[m]instanceof HTMLVideoElement){e="Cubemap source is not an instance of HTMLCanvasElement, HTMLImageElement or HTMLVideoElement.";b=!1;break}if(a[m].width!==f||a[m].height!==k){e="Cubemap sources do not all share the same dimensions.";b=!1;break}}e&&alert(e);return b};a.prototype._loadCubemapImages=function(a){var b=a.data,g=this;return new pc.promise.Promise(function(f,
